...
namespace System.Speech.Internal.Synthesis
{
	internal sealed class VoiceSynthesis : IDisposable
	{
		...
		private ManualResetEvent _evtPendingSpeak = new ManualResetEvent(initialState: false);

		private ManualResetEvent _evtPendingGetProxy = new ManualResetEvent(initialState: false);
		...
		private AutoResetEvent _workerWaitHandle = new AutoResetEvent(initialState: false);
					...
					case Action.SpeakText:
					{
						...
						try
						{
							...
							if (parametersSpeak2._prompt._exception == null)
							{
								...
								TTSVoice ttsVoice = (_currentVoice != null) ? _currentVoice : GetVoice(switchContext: false);
								...
								if (parametersSpeak2._textToSpeak != null)
								{
									...
									{
										TextFragmentEngine engine = new TextFragmentEngine(speakInfo, parametersSpeak2._textToSpeak, pexml: false, _resourceLoader, list);
		...
		private TTSVoice MatchVoice(CultureInfo culture, VoiceGender gender, VoiceAge age, int variant, bool switchContext, ref InstalledVoice viDefault)
		{
			...
			while (tTSVoice == null && list.Count > 0)
			{
				...
				if (tTSVoice == null)
				{
					...
					installedVoice.SetEnabledFlag(value: false, switchContext);
					...
					if ((assembly = Assembly.Load(voiceInfo.AssemblyName)) != null)
					{
						...
						foreach (Type type in array)
						{
							if (type.IsSubclassOf(typeof(TtsEngineSsml)))
							{
								...
								ttsEngineSsml = (assembly.CreateInstance(type.ToString(), ignoreCase: false, BindingFlags.Default, null, args, CultureInfo.CurrentUICulture, null) as TtsEngineSsml);
								break;
							}
						}
		...
		private ITtsEngineProxy GetComEngine(VoiceInfo voiceInfo)
		{
			...
			try
			{
				ObjectToken objectToken = ObjectToken.Open(null, voiceInfo.RegistryKeyPath, fCreateIfNotExist: false);
				if (objectToken != null)
				{
		...
		private void QueuePrompt(Prompt prompt)
		{
			...
			case SynthesisMediaType.Text:
				Speak(prompt._text, prompt, fIsXml: false);
				break;
			case SynthesisMediaType.Ssml:
				Speak(prompt._text, prompt, fIsXml: true);
				break;
		...
		{
			AddSpeakParameters(new Parameters(Action.SpeakText, new ParametersSpeak(null, prompt, isXml: false, audio)));
		}
