...
namespace System.Speech.Internal.Synthesis
{
	internal static class SsmlParser
	{
		...
		private static void ProcessSpeakElement(XmlReader reader, ISsmlParser engine, object voice)
		{
			...
			ProcessElement(reader, engine, "speak", possibleElements, ssmlAttributes, fIgnore: false, extraAttributes);
		...
		private static void ParseBreak(XmlReader reader, ISsmlParser engine, SsmlElement element, SsmlAttributes ssmAttributesParent, bool fIgnore)
		{
			...
			ProcessElement(reader, engine, sElement, (SsmlElement)0, ssmlAttributes, fIgnore: true, extraAttributes);
			engine.EndElement();
		}
		...
		private static void ParseDesc(XmlReader reader, ISsmlParser engine, SsmlElement element, SsmlAttributes ssmAttributesParent, bool fIgnore)
		{
			...
			ProcessElement(reader, engine, sElement, SsmlElement.Text, ssmlAttributes, fIgnore: true, extraAttributes);
			engine.EndElement();
		}
		...
		private static void ParseParagraph(XmlReader reader, ISsmlParser engine, SsmlElement element, SsmlAttributes ssmAttributesParent, bool fIgnore)
		{
			...
			ParseTextBlock(reader, engine, isParagraph: true, sElement, ssmAttributesParent, fIgnore);
		}
		...
		private static void ParseSentence(XmlReader reader, ISsmlParser engine, SsmlElement element, SsmlAttributes ssmAttributesParent, bool fIgnore)
		{
			...
			ParseTextBlock(reader, engine, isParagraph: false, sElement, ssmAttributesParent, fIgnore);
		}
		...
		private static void ParseVoice(XmlReader reader, ISsmlParser engine, SsmlElement element, SsmlAttributes ssmAttributesParent, bool fIgnore)
		{
			...
			while (reader.MoveToNextAttribute())
			{
				...
				if (reader.NamespaceURI.Length == 0)
				{
					...
					{
						CheckForDuplicates(ref dest2, reader);
						if (!SsmlParserHelpers.TryConvertGender(dest2, out VoiceGender gender))
						{
							flag = true;
						}
						else
					...
					case "age":
					{
						...
						if (!SsmlParserHelpers.TryConvertAge(dest5, out VoiceAge age))
						{
							flag = true;
						}
						else
		...
		private static ContourPoint[] ParseContour(string contour)
		{
			...
			{
				bool percent;
				while (num < array.Length && (num = NextChar(array, num, '(', skipDigit: false, out percent)) >= 0)
				{
					bool percent2;
					int num2 = NextChar(array, num, ',', skipDigit: true, out percent2);
					int num3 = NextChar(array, num2, ')', skipDigit: true, out percent);
					...
					if (!TryParseHz(contour.Substring(num2, num3 - (num2 + 1)), ref number2, acceptHzRelative: true, out bool isHz))
					{
						return null;
					}
					if (list.Count == 0)
		...
		private static bool ParseNumberHz(XmlReader reader, ref string attribute, string[] attributeValues, int[] attributeConst, ref ProsodyNumber number)
		{
			...
			if (num < 0)
			{
				if (!TryParseHz(attribute, ref number, acceptHzRelative: false, out bool _))
				{
					result = true;
				}
			}
		...
		{
			ProcessElement(reader, engine, null, (SsmlElement)0, ssmlAttributes, fIgnore: true, null);
		}
