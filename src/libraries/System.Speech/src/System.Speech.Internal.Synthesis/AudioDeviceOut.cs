...
namespace System.Speech.Internal.Synthesis
{
	internal class AudioDeviceOut : AudioBase, IDisposable
	{
		...
		private ManualResetEvent _evt = new ManualResetEvent(initialState: false);
		...
		internal static int GetDevicedId(string name)
		{
			...
			{
				if (GetDeviceName(i, out string prodName) == MMSYSERR.NOERROR && string.Compare(prodName, name, StringComparison.OrdinalIgnoreCase) == 0)
				{
					return i;
				}
			}
