...
namespace System.Speech.Recognition
{
	internal class RecognizerBase : IRecognizerInternal, IDisposable, ISpGrammarResourceLoader
	{
		...
		private AutoResetEvent _handlerWaitHandle = new AutoResetEvent(initialState: false);
		...
		internal void Initialize(SapiRecognizer recognizer, bool inproc)
		{
			...
			try
			{
				...
				{
					spPhoneticAlphabetSelection.SetAlphabetToUPS(fForceUPS: true);
				}
				else
				{
		...
		internal RecognitionResult Recognize(TimeSpan initialSilenceTimeout)
		{
			...
			try
			{
				...
				while (!completed && !_disposed)
				{
					if (!flag2)
					{
						flag = _handlerWaitHandle.WaitOne(timeout, exitContext: false);
						if (!flag)
						{
		...
		internal void SetInput(Stream stream, SpeechAudioFormatInfo audioFormat)
		{
			lock (SapiRecognizer)
			{
				...
				try
				{
					if (stream == null)
					{
						SapiRecognizer.SetInput(null, allowFormatChanges: false);
						_haveInputSource = false;
					}
					else
					{
						SapiRecognizer.SetInput(new SpAudioStreamWrapper(stream, audioFormat), allowFormatChanges: false);
		...
		internal void SetInputToDefaultAudioDevice()
		{
			lock (SapiRecognizer)
			{
				...
				try
				{
					spObjectTokenCategory.SetId("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Speech\\AudioInput", fCreateIfNotExist: false);
					spObjectTokenCategory.GetDefaultTokenId(out string ppszCoMemTokenId);
					ISpObjectToken spObjectToken = (ISpObjectToken)new SpObjectToken();
					try
					{
						spObjectToken.SetId(null, ppszCoMemTokenId, fCreateIfNotExist: false);
						SapiRecognizer.SetInput(spObjectToken, allowFormatChanges: true);
					}
					catch (COMException e)
					{
						throw ExceptionFromSapiCreateRecognizerError(e);
		...
		{
			Grammar grammar = (Grammar)grammarObject;
			this.LoadGrammarCompleted?.Invoke(this, new LoadGrammarCompletedEventArgs(grammar, grammar.LoadException, cancelled: false, null));
		}
		...
		private void InternalEmulateRecognizeAsync(string phrase, SpeechEmulationCompareFlags flag, bool useReco2, RecognizedWordUnit[] wordUnits)
		{
			...
			if (useReco2 || _supportsSapi53)
			{
				...
				spPhrase = ((wordUnits != null) ? SPPHRASE.CreatePhraseFromWordUnits(wordUnits, RecognizerInfo.Culture, out memHandles, out IntPtr coMem) : SPPHRASE.CreatePhraseFromText(phrase.Trim(), RecognizerInfo.Culture, out memHandles, out coMem));
				try
				{
		...
		private void EmulateRecognizedFailReportError(SAPIErrorCodes hr)
		{
			...
			{
				FireEmulateRecognizeCompletedEvent(null, _lastException, isRecognizeCancelled: true);
			}
		}
		...
		private void RecognizeAsyncWaitForGrammarsToLoad(object unused)
		{
			...
			if (ex != null || flag)
			{
				RecognizeCompletedEventArgs recognizeCompletedEventArgs = new RecognizeCompletedEventArgs(null, initialSilenceTimeout: false, babbleTimeout: false, inputStreamEnded: false, TimeSpan.Zero, ex, flag, null);
		...
		private void ProcessRecoOtherContextEvent()
		{
			...
			{
				FireEmulateRecognizeCompletedEvent(_lastResult, _lastException, isRecognizeCancelled: false);
			}
			lock (SapiRecognizer)
			{
				_audioState = AudioState.Silence;
