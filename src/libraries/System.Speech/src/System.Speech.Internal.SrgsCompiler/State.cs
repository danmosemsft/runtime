			...
			foreach (Arc item2 in list)
			{
				...
				if (semanticTagCount2 > 1)
				{
					for (int i = 1; i < semanticTagCount2 - 1; i++)
					{
						...
						num++;
						pWeights[iOffset++] = Arc.SerializeExtraEpsilonWithTag(streamBuffer, item2, isLast: true, num);
						iArcOffset++;
					}
					item2.SetArcIndexForTag(semanticTagCount2 - 1, iArcOffset, tagsCannotSpanOverMultipleArcs);
					pWeights[iOffset++] = item2.Serialize(streamBuffer, isLast: true, iArcOffset++);
					num++;
				}
			}
		}
